 ...
 */
export const fadeOut = (element, duration = 200, transitionTimingFunction = 'cubic-bezier(0.4, 0.0, 0.2, 1)') => {
 ...
 * @returns Promise resolving when the transition is done
 */
export const fadeIn = (element, duration = 200, transitionTimingFunction = 'cubic-bezier(0.4, 0.0, 0.2, 1)') => {
    return new Promise((resolve, reject) => {
        if (!element) {
            reject();
            return;
 ...
 */
export const createElement = (type, className, attributesObj = {}, html) => {
    ...
    Object.entries(attributesObj).forEach(([attrName, attrValue]) => element.setAttribute(attrName, attrValue));
    if (html)
        element.append(html);
    return element;
};
 ...
 */
export const trapFocus = (targetEl, initialFocusEl) => {
    const selectors = [
        // form elements
 ...
 */
export const loadImage = (targetImage) => {
 ...
 */
export const lazyLoadImages = (targetImages, observerOptions) => {
    const promises = [];
    const intersectionObserver = new IntersectionObserver((entries, observer) => {
        entries.forEach(entry => {
            ...
            loadImage(entry.target).then(() => {
                ...
                promises[promiseIndex].promise.resolve(entry.target);
            });
    ...
    targetImages.forEach(image => {
        ...
        promises.push({ image, promise: deferredPromise() });
    });
    return promises;
};
