 ...
 */
export const closeLightbox = async (
  galleryElement: GalleryElement,
  lightboxElement: LightBoxElement,
  focusTrap: ReturnType<typeof trapFocus>,
  currentGalleryItemIndex: number
) => {
  const lightboxImages = lightboxElement.querySelectorAll<LightBoxImage>(
    ELEMENTS.lightboxImage.selector
  );
  ...
  focusTrap.reset(
    galleryElement.querySelectorAll<GalleryElement>('.gallery-item')[currentGalleryItemIndex]
  );
 ...
 */
export const updateLightboxHeader = (
  lightbox: LightBoxElement,
  images: LightboxImageParams[],
  index: number
) => {
  const { title } = images[index];

  const lightboxTitle = lightbox.querySelector<HTMLDivElement>('.lightbox-title');
  const lightboxNumbers = lightbox.querySelector<HTMLDivElement>('.lightbox-numbers');

  if (lightboxTitle) lightboxTitle.textContent = title;
  if (lightboxNumbers) lightboxNumbers.textContent = `${index + 1}/${images.length}`;
};

/**
 * Returns the prev, current and next slide of the lightbox.
 */
export const getLightboxSlides = (lightboxElement: LightBoxElement) => {
  return {
    prev: lightboxElement.querySelector<LightboxSlideElement>(ELEMENTS.LightboxSlidePrev.selector),
    current: lightboxElement.querySelector<LightboxSlideElement>(
      ELEMENTS.LightboxSlideCurrent.selector
    ),
    next: lightboxElement.querySelector<LightboxSlideElement>(ELEMENTS.LightboxSlideNext.selector),
  };
};
...
export const createLightbox = () => {
  ...
  let lightbox = document.querySelector<LightBoxElement>(ELEMENTS.lightbox.selector);
 ...
 */
export const getImages = (galleryElement: GalleryElement) => {
  // get image params of all items in a gallery
  const getGalleryItemParams = (galleryItems: GalleryItemElement[]) => {
    return galleryItems.reduce<LightboxImageParams[]>((acc, element) => {
      const currentImageEl = element.querySelector<GalleryImage>(ELEMENTS.galleryImage.selector);
      return [
        ...acc,
        {
  ...
  const galleryItemElements = galleryElement.querySelectorAll<GalleryItemElement>(
    ELEMENTS.galleryItem.selector
  );
 ...
 */
export const getTargetElement = (target: GalleryElement | string) => {
  const targetElement =
    target instanceof HTMLElement ? target : document.querySelector<GalleryElement>(target);
  // eslint-disable-next-line no-console
  if (!targetElement) console.warn('gallery error: target element not found');
 ...
 */
export const loadSlideImage = (
  images: LightboxImageParams[],
  targetSlide: LightboxSlideElement | null,
  index: number
) => {
  const currentImage = targetSlide?.querySelector<LightBoxImage>(ELEMENTS.lightboxImage.selector);
  const spinner = targetSlide?.querySelector<HTMLElement>(ELEMENTS.lightboxSpinner.selector);

  currentImage?.setAttribute('data-src', images[index].src);
  currentImage?.setAttribute('data-srcset', images[index].srcset);

  if (!currentImage || !spinner) return;
 ...
 */
export const initGalleryItems = (galleryElement: GalleryElement) => {
  const galleryItems = galleryElement.querySelectorAll<GalleryItemElement>(
    ELEMENTS.galleryItem.selector
  );
  const galleryThumbs = galleryElement.querySelectorAll<GalleryImage>(
    ELEMENTS.galleryImage.selector
  );
  ...
  return Array.from(galleryItems).map(itemEl => ({
    element: itemEl,
    title: itemEl.querySelector<GalleryImage>(ELEMENTS.galleryImage.selector)?.dataset.title,
  }));
};
