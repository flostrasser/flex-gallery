...
import { fadeIn, trapFocus, lazyLoadImages, disablePageScroll } from './lib/domUtils.js';
...
export const flexGallery = (target: GalleryElement | string, customOptions: Options = {}) => {
  // options
  ...
  // global variables
  let focusTrap: ReturnType<typeof trapFocus>;
  let currentGalleryItemIndex: number;
  ...
  function transformSlide(slide: LightboxSlideElement | null, translateX: string, opacity: number) {
    if (!slide) return;
  ...
  function showNextSlide(lightboxElement: LightBoxElement) {
  ...
  function showPrevSlide(lightboxElement: LightBoxElement) {
    const slides = getLightboxSlides(lightboxElement);
  ...
  function updateLightbox(
    ...
    setTimeout(() => {
      ...
      Object.values(slides).forEach(element => {
        if (element) element.style.transitionDuration = '0ms';
      });

      let index;

      if (newSlide === SlideType.next) {
        if (slides.prev) slides.prev.dataset.state = 'next';
        if (slides.next) slides.next.dataset.state = 'current';
        if (slides.current) slides.current.dataset.state = 'prev';
        ...
        if (slides.prev) slides.prev.dataset.state = 'current';
        if (slides.current) slides.current.dataset.state = 'next';
        if (slides.next) slides.next.dataset.state = 'prev';
  ...
  const isGalleryOpen = (galleryElement: GalleryElement) => {
  ...
  const initLightBoxEvents = (
    ...
    const lightboxSlides = lightboxElement.querySelectorAll<LightboxSlideElement>(
      ELEMENTS.LightboxSlide.selector
    );
    lightboxSlides.forEach(slide => {
      slide.addEventListener('click', event => {
        if (!isGalleryOpen(galleryElement)) return;
        const isClickOnSlide = event.currentTarget === event.target && !wasSwiping;
        if (!isClickOnSlide) return;
        closeLightbox(galleryElement, lightboxElement, focusTrap, currentGalleryItemIndex);
      });
    });

    // close lightbox when clicking on close button
    const closeBtn = lightboxElement.querySelector<LightBoxCloseButton>(
      ELEMENTS.lightboxClose.selector
    );
    ...
    const arrowLeft = lightboxElement.querySelector<LightBoxArrow>(
      ELEMENTS.lightboxArrowLeft.selector
    );
    ...
    const arrowRight = lightboxElement.querySelector<LightBoxArrow>(
      ELEMENTS.lightboxArrowRight.selector
    );
  ...
  const showInitialImage = (
    ...
    const currentImage = slides.current?.querySelector<LightBoxImage>(
  ...
  const openLightbox = (
    targetItem: GalleryItemElement | null,
    lightbox: LightBoxElement,
    images: LightboxImageParams[],
    index: number
  ) => {
    const targetGallery = targetItem?.closest<GalleryElement>(ELEMENTS.gallery.selector);
    ...
    // get close button
    const closeButton = lightbox.querySelector<LightBoxCloseButton>(
      ELEMENTS.lightboxClose.selector
    );
  ...
  const initGalleryItemEvents = (
    galleryItems: GalleryItemParams[],
    lightboxElement: LightBoxElement,
    images: LightboxImageParams[]
  ) => {
    galleryItems.forEach((item, index) => {
      ...
      item.element.addEventListener('click', event => {
        currentGalleryItemIndex = index;
        openLightbox(event.currentTarget as GalleryItemElement, lightboxElement, images, index);
      });
